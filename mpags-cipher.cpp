...
int main(int argc, char* argv[])
{
  ...
  ProgramSettings arguments {false, false, "","","", mode};
  ...
  */
  ...
  /*  if ( ! arguments.cipher_key.empty() ) {
    ...
    for ( const auto& elem : arguments.cipher_key ) {
      if ( ! std::isdigit(elem) ) {
	std::cerr << "[error] cipher key must be an unsigned long integer for Caesar cipher,\n"
	          << "        the supplied key (" << arguments.cipher_key << ") could not be successfully converted" << std::endl;
	return 1;
      }
    }
    caesar_key = std::stoul(arguments.cipher_key);
    }*/

  CaesarCipher myCipher(arguments.cipher_key);

  // Run the Caesar cipher (using the specified key and encrypt/decrypt flag) on the input text
  //  std::string outputText { runCaesarCipher( inputText, caesar_key, arguments.encrypt ) };
